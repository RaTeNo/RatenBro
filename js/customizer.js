...
( function( $ ) {
	...
	wp.customize( 'raten_color_main', function( value ) {
		value.bind( function( to ) {
	...
	wp.customize( 'raten_color_menu_bg', function( value ) {
		value.bind( function( to ) {
	...
	wp.customize( 'raten_color_text', function( value ) {
		value.bind( function( to ) {
			$( 'body' ).css( 'color', to );
		} );
	});

	wp.customize( 'raten_color_link', function( value ) {
		value.bind( function( to ) {
			$( 'a, .spanlink, .comment-reply-link, .pseudo-link' ).css( 'color', to );
		} );
	});

	wp.customize( 'raten_color_link_hover', function( value ) {
		value.bind( function( to ) {
			$( 'a:hover, a:focus, a:active, .spanlink:hover, .comment-reply-link:hover, .pseudo-link:hover' ).css( 'color', to );
		} );
	});

	wp.customize( 'raten_color_logo', function( value ) {
		value.bind( function( to ) {
    ...
    // Цвет ссылок в меню
	wp.customize( 'raten_color_menu', function( value ) {
		value.bind( function( to ) {
     ...
     * Типографика
     *******************************************************************/
	wp.customize( 'raten_typography_font_size', function( value ) {
		value.bind( function( to ) {
            $( 'body' ).css( 'font-size', to + 'px' );
		} );
	});

	wp.customize( 'raten_typography_line_height', function( value ) {
		value.bind( function( to ) {
    ...
    // текст на главной
    wp.customize( 'raten_structure_home_h1', function( value ) {
        value.bind( function( to ) {
            $( '.home-header' ).html( to );
        } );
    });
    ...
    // текст на главной
    wp.customize( 'raten_structure_home_text', function( value ) {
        value.bind( function( to ) {
            $( '.home-text' ).html( to );
        } );
    });
